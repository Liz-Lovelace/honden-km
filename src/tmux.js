...
export async function spawn_console(session, command, width, height) {
...
export async function kill_console(session) {
  await execPromise(`tmux kill-session -t ${session}`);
}

export async function send_keys(session, love_keycode, modifier_keys = []) {
  const tmux_keycode = love_to_tmux_key(love_keycode, modifier_keys);
  await execPromise(`tmux send-keys -t ${session} ${tmux_keycode}`);
}

export async function resize_window(session, width, height) {
  await execPromise(`tmux resize-window -x ${width} -y ${height} -t ${session}`);
}

export async function capture_pane(session) {
  await execPromise(`tmux capture-pane -t ${session}`);
  await execPromise(`tmux save-buffer /tmp/pane_content.txt`);
  const cursor_position = await execPromise(`tmux display-message -p -F "#{cursor_x},#{cursor_y}" -t ${session}`);

  const [x, y] = cursor_position.trim().split(',').map(Number);
  const content = await fs.readFile("/tmp/pane_content.txt", "utf8");
